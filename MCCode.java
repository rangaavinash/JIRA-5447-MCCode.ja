...
public class MCCode extends HttpServlet {
	...
	public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		String msFlag = null;
		try { 
			conn = SharedConnectionPool.getInstance().getConnection();// Added for JIRA-5447
			...
			if (req.getParameter("entry") != null) {
				if (!(req.getParameter("entry").equals(""))) {
					...
					if (!DataValidation.isAlphabets(req.getParameter("entry"))) {
						arrErrorList.add(LocaleSet.getProperty("cms.PreAuth.S220.exceptionProcessingMode") + " "
								+ LocaleSet.getProperty("cms.parameters.s15.exceptionIsInvalid"));
					} else {
			...
			if (req.getParameter("tranType") != null)
				if (!(req.getParameter("tranType").equals(""))) {
					...
					if (!DataValidation.isAlphabets(req.getParameter("tranType"))) {
						arrErrorList.add(LocaleSet.getProperty("cms.parameters.s17.exceptionProcessingMode") + " "
								+ LocaleSet.getProperty("cms.parameters.s15.exceptionIsInvalid"));
					} else {
			...
			if (tranType.equals("ADD")) { // ADD if 01
				if (entry.equals("First")) {
					...
					try {
						...
						moLogFile.writeToLog(this.getClass().getName(), 223,
								"---mccode---" + req.getParameter("mccode"));
						if (req.getParameter("mccode") != null) {
							if (!(req.getParameter("mccode").equals(""))) {
								if (!DataValidation.isNumeric(req.getParameter("mccode"))) {
									arrErrorList.add(LocaleSet.getProperty("cms.PreAuth.S66.textMercCatCode") + " "
											+ LocaleSet.getProperty("cms.parameters.s15.exceptionMustBeNumeric"));
								} else {
						...
						moLogFile.writeToLog(this.getClass().getName(), 223,
								"---mcdesc---" + req.getParameter("mcdesc"));
						if (req.getParameter("mcdesc") != null)
							if (!(req.getParameter("mcdesc").equals(""))) {
								...
								if (!DataValidation.isAlphaNumUnscSomeSpecial(req.getParameter("mcdesc"))) {
									arrErrorList.add(LocaleSet.getProperty("cms.PreAuth.S66.textMercCatCodeDesc") + " "
											+ LocaleSet.getProperty(
													"cms.parameters.s15.exceptionMustBeAlphaNumericWithAllowableUndersocreAndSpace"));
								} else {
						...
						moLogFile.writeToLog(this.getClass().getName(), 223,
								"---ActivationStatus---" + req.getParameter("ActivationStatus"));
						if (req.getParameter("ActivationStatus") != null)
							if (!(req.getParameter("ActivationStatus").equals(""))) {
								...
								if (!DataValidation.isAlphaYesNo(req.getParameter("ActivationStatus"))) {
									arrErrorList.add(LocaleSet.getProperty("cms.PreAuth.S66.buttonActivateStatus") + " "
											+ LocaleSet.getProperty("cms.parameters.s15.exceptionIsInvalid"));
								} else {
						...
						if (arrErrorList.size() > 0) {
							...
							if (a > 0) {
								parameter = "<Label  CLASS ='Label6'><b>"
										+ LocaleSet
												.getProperty("cms.PreAuth.S322.statusMerchantCategoryCodeAlreadyExist")
										+ "</b></label>";
							...
							// Commented by Trivikram on 15-05-2012 by no need to create connection again,
							...
							if (a > 0) {
								parameter = "<Label  CLASS ='label6'><b>"
										+ LocaleSet.getProperty(
												"cms.PreAuth.S322.statusMerchantCategoryCodeDescriptionAlreadyExist")
										+ "</b></label>";
								req.setAttribute("parameter", "" + parameter);
								rd = req.getRequestDispatcher("/MCCodeRegADD.jsp?entry=First");
								rd.forward(req, res);
								...
								/**
								 * if(mccode.length() < 4) { String pad="0000";
								...
								CmsUtils.close(pStmt); // Added for JIRA-5447
								if (insert == 1) {
									parameter = "<Label  CLASS ='label6'>"
											+ LocaleSet.getProperty("cms.PreAuth.S322.statusAddedSuccessfully")
											+ "</label>";
									req.setAttribute("parameter", "" + parameter);
									...
									req.setAttribute("heading",
											LocaleSet.getProperty("cms.PreAuth.S322.statusMerchantCategoryCode"));
									req.setAttribute("status",
											"<Label  CLASS ='Label6'>" + LocaleSet.getProperty(
													"cms.PreAuth.S322.statusMerchantCategoryCodeRegistrationFailed")
													+ "</label>");
						...
						try {
							 ...
							 * if(rs != null) rs.close();
							 * 
							 * if(conn != null) SharedConnectionPool.getInstance().free(conn);
							 */
							CmsUtils.close(rs); // Added for JIRA-5447
							CmsUtils.close(pStmt); // Added for JIRA-5447
							CmsUtils.close(conn); // Added for JIRA-5447
						} catch (Exception e) {
							moLogFile.writeToLog(this.getClass().getName(), 155,
									"Exception :------> " + e.getMessage());
							clsLogFile.printStackTrace(e);
						}
					}
				}
			} else if (tranType.equals("EDIT")) { // add if end edit if 02
				if (entry.equals("First")) {
					...
					try {
						...
						if (arrErrorList.size() > 0) {
							...
							try {
								...
								rs = pStmt.executeQuery();
								if (rs.next()) {
									mccdesc = rs.getString(1);
									ActivationStatus = rs.getString(2);
									msStatus = rs.getString(3);

								}
								CmsUtils.close(rs); // Added for JIRA-5447
								CmsUtils.close(pStmt); // Added for JIRA-5447
							} catch (Exception e) {
								moLogFile.writeToLog(this.getClass().getName(), 246,
										"Exception :------> " + e.getMessage());
								clsLogFile.printStackTrace(e);
								...
								try {
									...
									moLogFile.writeToLog(this.getClass().getName(), 278,
											"Exception while Closing Conn :------> " + e.getMessage());
									clsLogFile.printStackTrace(e);
									throw new Exception(e);
								}
							...
							if (mccdesc == null || ActivationStatus == null || msStatus == null) {
								...
								// String Linked = "Y";
								try {
									sql = " SELECT COUNT(*) " + " FROM MCCODE_GROUP A, MCCODE B  "
											+ " WHERE B.MCCODE = A.MCCODE " + " AND  B.MCCODE =  ? "
											+ " AND ACT_INST_CODE = ? ";

									moLogFile.writeToLog(this.getClass().getName(), 226, "mccdesc :---->" + mccdesc);
									moLogFile.writeToLog(this.getClass().getName(), 226,
											"ActivationStatus :---->" + ActivationStatus);
									moLogFile.writeToLog(this.getClass().getName(), 226,
											"miInstCode :---->" + miInstCode);
					...
					try {
						...
						if (arrErrorList.size() > 0) {
							...
							CmsUtils.close(pStmt); // Added for JIRA-5447
							if (count > 0) {
								parameter = "<Label  CLASS ='Label6'>" + LocaleSet.getProperty(
										"cms.PreAuth.S322.statusMerchantCategoryCodeAlreadyExist") + "</label>";
								req.setAttribute("mccode", mccode);
								req.setAttribute("mcdesc", mccdesc);
								req.setAttribute("ActivationStatus", ActivationStatus);
								req.setAttribute("parameter", "" + parameter);
								...
								CmsUtils.close(pStmt); // Added for JIRA-5447
								if (insert == 1) {
									parameter = "<center><Label  CLASS ='Label6'>"
											+ LocaleSet.getProperty("cms.PreAuth.S322.statusMerchantCategoryCode") + " "
											+ mccode + " "
											+ LocaleSet.getProperty("cms.PreAuth.S322.statusModifiedSuccessfully")
											+ "</label></center>";
									req.setAttribute("parameter", "" + parameter);
									req.setAttribute("mccode", "" + mccode);
									req.setAttribute("mcdesc", "" + mcdesc);
									...
									parameter = "<center><Label  CLASS ='Label6'>"
											+ LocaleSet.getProperty(
													"cms.PreAuth.S322.statusMerchantCategoryCodeModificationFailed")
											+ "</label></center>";
									req.setAttribute("parameter", parameter);
									req.setAttribute("mccode", mccode);
									req.setAttribute("mcdesc", mcdesc);
...

						{
							...
							moLogFile.writeToLog(this.getClass().getName(), 362,
									"Exception From Dynamic Error :--------> " + ed.getMessage());
						}
						...
						try {
							...
							moLogFile.writeToLog(this.getClass().getName(), 371,
									"Exception while closing Conn :--->" + e.getMessage());
							clsLogFile.printStackTrace(e);
						}
					}
				}
			} else if (tranType.equals("DELETE")) { // edit if end delete if 03
				if (entry.equals("First")) {
					...
					try {
						...
						if (arrErrorList.size() > 0) {
							 ...
							 * req.setAttribute("ERROR","error");
							 * rd=req.getRequestDispatcher("/MCModification.jsp"); rd.forward(req,res); }
							 */
							else {
								req.setAttribute("mccode", mccode);
								req.setAttribute("mcdesc", mccdesc);
...

						{
							...
							moLogFile.writeToLog(this.getClass().getName(), 362,
									"Exception Ora Dyna Message :--------> " + ed.getMessage());
						}
						...
						if (msoraErrMsg.length() > 0) {
							...
							req.setAttribute("status",
									"<label class=label6>"
											+ LocaleSet.getProperty(
													"cms.PreAuth.S322.statusMerchantcategoryCodedoesntexists")
											+ "</label>");
						}
					...
					try {
						...
						if (arrErrorList.size() > 0) {
							...
							if (rs.next()) {
								...
								if (tl > 0) {
									...
									req.setAttribute("status",
											"<label class=label6>" + LocaleSet.getProperty(
													"cms.PreAuth.S322.statusDeleteFailedMerchantCategoryCodeislinked")
													+ "</label>");
									...
									if (delete == 2292) {
										...
										req.setAttribute("status",
												"<label class=label6>"
														+ LocaleSet.getProperty(
																"cms.PreAuth.S322.statusMerchantCategoryCode")
														+ " " + mccode + " "
														+ LocaleSet.getProperty("cms.PreAuth.S322.statusDeleteFailed")
														+ "</label>");
										rd = req.getRequestDispatcher("/Status.jsp");
										rd.forward(req, res);
										...
										req.setAttribute("heading",
												LocaleSet.getProperty("cms.PreAuth.S322.statusMerchantCategoryCode"));
										req.setAttribute("status", "<label class=label6>"
												+ LocaleSet.getProperty("cms.PreAuth.S322.statusMerchantCategoryCode")
												+ " " + mccode + " "
												+ LocaleSet.getProperty("cms.PreAuth.S322.statusDeletedSuccessfully")
												+ "</label>");
										rd = req.getRequestDispatcher("/Status.jsp");
										rd.forward(req, res);
									}
...

						{
							...
							moLogFile.writeToLog(this.getClass().getName(), 362,
									"Exception Ora Dyna Message :--------> " + e.getMessage());
						}
						...
						if (msoraErrMsg.length() > 0) {
							...
							req.setAttribute("status",
									"<label class=label6>"
											+ LocaleSet.getProperty(
													"cms.PreAuth.S322.statusMerchantCategoryCodeDeleteFailed")
											+ "</label>");
						}
						...
						try {
							...
							moLogFile.writeToLog(this.getClass().getName(), 490,
									"Exception while closing CONN :------> " + e.getMessage());
							clsLogFile.printStackTrace(e);
						}
					}
				}
			} else if (tranType.equals("VIEW")) { // delete if end view if 04
				if (entry.equals("First")) {
					// out.println("First");
					...
					try {
						...
						req.setAttribute("heading",
								LocaleSet.getProperty("cms.PreAuth.S322.statusMerchantCategoryCode"));
						req.setAttribute("status",
								LocaleSet.getProperty("cms.PreAuth.S322.statusMerchantCategoryCodeViewFailed"));
						rd = req.getRequestDispatcher("/Status.jsp");
						// rd.forward(req,res);
					} finally {
						try {
			...
			moLogFile.writeToLog(this.getClass().getName(), 1048, "Exception :-------> " + e.getMessage());
			clsLogFile.printStackTrace(e);
		} finally {
			try {
				CmsUtils.close(rs);
				...
				moLogFile.writeToLog(this.getClass().getName(), 1056,
						"Exception in closing conection:-------> " + e.getMessage());
				clsLogFile.printStackTrace(e);
			}
		}
	}
